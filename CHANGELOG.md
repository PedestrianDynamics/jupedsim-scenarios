...
- **Placement-failure errors now reference the distribution id, not an
  opaque enumerate index.** `_initialize_complete_config` raised
  ...
  log line and error in the legacy spawn path (matching the journey-v2
  path, which already used ``dist_key``). Regression test pins the new
  contract.
